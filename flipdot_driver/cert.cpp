#include <Arduino.h>
#include "cert.h"
// this file is autogenerated - any modification will be overwritten
// unused symbols will not be linked in the final binary
// generated on 2025-03-29 09:14:46
// by ['./cert.py', '-s', '', '-n', 'lambda']


////////////////////////////////////////////////////////////
// certificate chain for kzhdqgau4zmvo346mibt5cob7y0pdlsw.lambda-url.eu-west-1.on.aws:443



// CN: *.lambda-url.eu-west-1.on.aws => name: __lambda_url_eu_west_1_on_aws
// not valid before: 2024-11-27 00:00:00+00:00
// not valid after:  2025-12-27 23:59:59+00:00
const char fingerprint___lambda_url_eu_west_1_on_aws [] PROGMEM = "56:e9:72:4f:6c:ed:84:f7:f5:50:31:da:8f:dc:ce:25:f6:6a:b0:9f";
const char pubkey___lambda_url_eu_west_1_on_aws [] PROGMEM = R"PUBKEY(
-----BEGIN PUBLIC KEY-----

cQIDAQAB
-----END PUBLIC KEY-----
)PUBKEY";

// http://crt.r2m03.amazontrust.com/r2m03.cer
// CN: Amazon RSA 2048 M03 => name: Amazon_RSA_2048_M03
// not valid before: 2022-08-23 22:26:04+00:00
// not valid after:  2030-08-23 22:26:04+00:00
const char cert_Amazon_RSA_2048_M03 [] PROGMEM = R"CERT(
-----BEGIN CERTIFICATE-----

i02f2ivH2vxkjDLltSpe2fiC
-----END CERTIFICATE-----
)CERT";

// http://crt.rootca1.amazontrust.com/rootca1.cer
// CN: Amazon Root CA 1 => name: Amazon_Root_CA_1
// not valid before: 2015-05-25 12:00:00+00:00
// not valid after:  2037-12-31 01:00:00+00:00
const char cert_Amazon_Root_CA_1 [] PROGMEM = R"CERT(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
)CERT";

// http://crt.rootg2.amazontrust.com/rootg2.cer
// CN: Starfield Services Root Certificate Authority - G2 => name: Starfield_Services_Root_Certificate_Authority___G2
// not valid before: 2009-09-02 00:00:00+00:00
// not valid after:  2034-06-28 17:39:16+00:00
const char cert_Starfield_Services_Root_Certificate_Authority___G2 [] PROGMEM = R"CERT(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
)CERT";

// http://x.ss2.us/x.cer
// CN:  => name:
// not valid before: 2004-06-29 17:39:16+00:00
// not valid after:  2024-06-29 17:39:16+00:00
const char cert_ [] PROGMEM = R"CERT(
-----BEGIN CERTIFICATE-----

FxdzPcwl
-----END CERTIFICATE-----
)CERT";




// end of certificate chain for kzhdqgau4zmvo346mibt5cob7y0pdlsw.lambda-url.eu-west-1.on.aws:443
////////////////////////////////////////////////////////////

